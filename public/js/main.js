...
function renderGames() {
  ...
  filtered.forEach((game, i) => {
    ...
    // ── وضع التعديل: إضافة أزرار التعديل والحذف ──
    if (editModeActive) {
      card.style.position = 'relative';
      card.classList.add('em-edit-mode-card');
      const editOverlay = document.createElement('div');
      editOverlay.className = 'em-card-actions';
      editOverlay.innerHTML = `
        <button class="em-card-btn em-edit-btn"
          onclick="event.stopPropagation(); openEditGameModal('${game.id}')"
          title="تعديل اللعبة" aria-label="تعديل">
          <i class="fas fa-pencil-alt"></i>
        </button>
        <button class="em-card-btn em-delete-btn"
          onclick="event.stopPropagation(); openDeleteModal('${game.id}')"
          title="حذف اللعبة" aria-label="حذف">
...
function toggleEditMode() {
  ...
  if (editModeActive) {
    ...
    label.textContent = 'إيقاف التعديل';
    toolbar.style.display = 'block';
    toolbar.classList.add('em-toolbar-visible');
    showToast('وضع التعديل مفعّل ✏️', 'info');
  } else {
    btn.classList.remove('edit-mode-active');
    label.textContent = 'تعديل';
    toolbar.style.display = 'none';
    toolbar.classList.remove('em-toolbar-visible');
    showToast('تم إيقاف وضع التعديل', 'info');
  }

  renderGames(); // إعادة رسم الكروت مع/بدون أزرار التعديل
}


// ── فتح modal الإضافة ────────────────────────────────────
function openAddGameModal() {
  document.getElementById('emGameId').value          = '';
  document.getElementById('emNameAr').value          = '';
  document.getElementById('emName').value            = '';
  document.getElementById('emPrice').value           = '';
  document.getElementById('emSize').value            = '';
  document.getElementById('emCategory').value        = 'Action';
  document.getElementById('emHardDrive').value       = '1';
  document.getElementById('emImage').value           = '';
  document.getElementById('emDescription').value     = '';

  document.getElementById('editModalTitle').innerHTML = `
    <i class="fas fa-plus-circle" style="color:var(--clr-gold); margin-left:0.5rem;"></i>
    إضافة لعبة جديدة`;
  document.getElementById('emSaveBtnLabel').textContent = 'إضافة اللعبة';
...
function openEditGameModal(gameId) {
  ...
  document.getElementById('emGameId').value          = game.id;
  document.getElementById('emNameAr').value          = game.nameAr || '';
  document.getElementById('emName').value            = game.name || '';
  document.getElementById('emPrice').value           = game.price || '';
  document.getElementById('emSize').value            = game.size || '';
  document.getElementById('emCategory').value        = game.category || 'Action';
  document.getElementById('emHardDrive').value       = game.hardDrive || '1';
  document.getElementById('emImage').value           = game.image || '';
  document.getElementById('emDescription').value     = game.description || '';

  document.getElementById('editModalTitle').innerHTML = `
    <i class="fas fa-pencil-alt" style="color:var(--clr-gold); margin-left:0.5rem;"></i>
    تعديل: ${game.nameAr || game.name}`;
  document.getElementById('emSaveBtnLabel').textContent = 'حفظ التعديلات';

  openEMOverlay();
}

function openEMOverlay() {
  document.getElementById('editGameOverlay').classList.add('em-active');
  document.body.style.overflow = 'hidden';
}

function closeEditGameModal() {
  document.getElementById('editGameOverlay').classList.remove('em-active');
  document.body.style.overflow = '';
}

function handleEditOverlayClick(e) {
  if (e.target.id === 'editGameOverlay') closeEditGameModal();
}

// ── حفظ اللعبة (إضافة أو تعديل) ────────────────────────
async function saveGameFromCatalog(e) {
  e.preventDefault();

  const gameId      = document.getElementById('emGameId').value.trim();
  const name        = document.getElementById('emName').value.trim();
  const nameAr      = document.getElementById('emNameAr').value.trim();
  const price       = document.getElementById('emPrice').value.trim();
  const size        = document.getElementById('emSize').value.trim();
  const category    = document.getElementById('emCategory').value;
  const hardDrive   = document.getElementById('emHardDrive').value;
  const image       = document.getElementById('emImage').value.trim();
  const description = document.getElementById('emDescription').value.trim();

  if (!name || !nameAr || !price || !size) {
    showToast('يرجى ملء الحقول الإلزامية', 'error');
    return;
  }

  const saveBtn = document.getElementById('emSaveBtn');
  const saveLbl = document.getElementById('emSaveBtnLabel');
  saveBtn.disabled = true;
  saveLbl.textContent = 'جاري الحفظ...';

  const payload = { name, nameAr, price: Number(price), size, category, hardDrive, image, description };

  try {
    let res;
    if (gameId) {
      // تعديل لعبة موجودة
      res = await fetch(`/api/games/public-update/${gameId}`, {
        method:  'PUT',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify(payload)
      });
      ...
      // إضافة لعبة جديدة
      res = await fetch('/api/games/public-add', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify(payload)
      });
    ...
    if (res.ok) {
      ...
      if (gameId) {
        // نحدّث اللعبة في الـ array المحلي
        const idx = allGames.findIndex(g => g.id === gameId);
        if (idx !== -1) allGames[idx] = { ...allGames[idx], ...saved };
        showToast(`✏️ تم تعديل "${saved.nameAr || saved.name}" بنجاح`);
      } else {
        // نضيف اللعبة الجديدة
        allGames.unshift(saved);
        showToast(`✅ تم إضافة "${saved.nameAr || saved.name}" بنجاح`);
      }

      renderGames();
    } else {
      const err = await res.json().catch(() => ({}));
      showToast(err.error || 'حدث خطأ أثناء الحفظ', 'error');
    }
  } catch {
    showToast('فشل الاتصال بالخادم', 'error');
  } finally {
    saveBtn.disabled = false;
    saveLbl.textContent = gameId ? 'حفظ التعديلات' : 'إضافة اللعبة';
  }
}
...
function openDeleteModal(gameId) {
  ...
  if (!game) return;
  gameToDelete = gameId;
  document.getElementById('deleteGameName').textContent = `"${game.nameAr || game.name}"`;
  document.getElementById('deleteConfirmOverlay').classList.add('em-active');
  document.body.style.overflow = 'hidden';
}

function closeDeleteModal() {
  document.getElementById('deleteConfirmOverlay').classList.remove('em-active');
  document.body.style.overflow = '';
  gameToDelete = null;
}
...
async function confirmDelete() {
  ...
  try {
    const res = await fetch(`/api/games/public-delete/${gameToDelete}`, { method: 'DELETE' });

    if (res.ok) {
      const game = allGames.find(g => g.id === gameToDelete);
      const gameName = game ? (game.nameAr || game.name) : 'اللعبة';

      // إزالة من الـ cart إذا موجودة
      cart = cart.filter(c => c.id !== gameToDelete);
      saveCart();
      updateCartBadge();
      ...
      closeDeleteModal();
      renderGames();
      showToast(`🗑️ تم حذف "${gameName}" بنجاح`);
    } else {
      const err = await res.json().catch(() => ({}));
    ...
    btn.disabled = false;
    btn.innerHTML = '<i class="fas fa-trash-alt"></i> نعم، احذف اللعبة';
    gameToDelete = null;
  }
}

// ── Escape key للـ modals الجديدة ──────────────────────────
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') {
    if (document.getElementById('editGameOverlay')?.classList.contains('em-active'))    closeEditGameModal();
    if (document.getElementById('deleteConfirmOverlay')?.classList.contains('em-active')) closeDeleteModal();
  }
});
